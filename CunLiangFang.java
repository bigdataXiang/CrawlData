...
public class Cunliangfang {
	...
	public static void getStoragehouse(int pageCount) 
	{
		...
		try {
			 ...
			 * </a>
			 */
			NodeFilter filter = new AndFilter(new TagNameFilter("a"), new AndFilter(new HasParentFilter(new AndFilter(new TagNameFilter("td"), new HasParentFilter(new TagNameFilter("tr")))), new HasChildFilter(new TagNameFilter("img"))));
			NodeList nodes = parser.extractAllNodesThatMatch(filter);
			for (int n = 0; n < 10; n ++)
			{
